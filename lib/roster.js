export function buildRosterText(templates) {
    ...
    const lines = enabled.map((t) => {
        ...
        return `- ${t.label} (id ${t.id}): provider=${t.provider}, role="${t.role}", permissionMode=${t.permissionMode}, memberProvider=${t.memberProvider}${model}${depth}`;
    });
    return [
