...
function callerAgent(exec) {
    ...
    if (!agent)
        throw new Error('no calling agent available to launch a sub-agent');
    return agent;
}
function text(value) {
...
export function registerSubagentTemplateTools(ctx, manager) {
    const dispose = [];
    const listTool = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text(`Sub-agent templates: ${value.templates.length ? value.templates.map((t) => t.id).join(', ') : 'none'}`),
        },
        execute: async (args) => {
    ...
    const createTool = defineTool({
        ...
        parameters: {
            ...
            member_provider: { type: 'string', enum: ['spawn', 'fork'], description: 'Member provider for agent-teams; defaults to fork.' },
        ...
        output: {
            ...
            render: (_args, value) => text(`Template "${value.id}" created (${value.label}, ${value.permission_mode}, enabled=${value.enabled}).`),
        },
        execute: async (args) => {
            const created = await manager.create({
                ...
                permissionMode: (args.permission_mode ?? 'readonly'),
                memberProvider: (args.member_provider ?? 'fork'),
    ...
    const enableTool = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text(`Template "${value.id}" is now enabled=${value.enabled} (${value.permission_mode}).`),
        },
        execute: async (args) => {
    ...
    const launchTool = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text(`Launched template "${value.template_id}" as child sub-agent "${value.child_id}".`),
        },
